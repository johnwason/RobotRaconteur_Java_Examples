...
public class Webcam_stub extends ServiceStub implements Webcam, async_Webcam {
    ...
    return MessageElementUtil.<String>castDataAndDispose(rr_innerstub.propertyGet("Name"));
    }
    ...
    return RobotRaconteurNode.s().<WebcamImage>unpackStructureDispose(rr_me.<MessageElementStructure>castData());
    }
    finally {
    rr_param.delete();
    ...
    return RobotRaconteurNode.s().<WebcamImage_size>unpackStructureDispose(rr_me.<MessageElementStructure>castData());
    }
    finally {
    rr_param.delete();
    if (rr_me!=null) rr_me.delete();
    ...
    public void async_set_Name(String value, Action1<RuntimeException> rr_handler, int rr_timeout)
    {
    MessageElement m=null;
    try {
    m=MessageElementUtil.newMessageElementDispose("value",value);
    rr_async_PropertySet("Name",m,new rrend_async_set_Name(),rr_handler,rr_timeout);
    }
    finally {
    if (m!=null) m.delete();
    }
    }
    protected class rrend_async_set_Name implements Action3<MessageElement,RuntimeException,Object> {
    ...
    WebcamImage rr_ret;
    try {
    rr_ret=RobotRaconteurNode.s().<WebcamImage>unpackStructureDispose(ret.<MessageElementStructure>castData());
    } catch (RuntimeException err2) {
    ...
    try {
    rr_ret=RobotRaconteurNode.s().<WebcamImage_size>unpackStructureDispose(ret.<MessageElementStructure>castData());
    } catch (RuntimeException err2) {
    rr_handler.action(null,err2);
    return;
    }
